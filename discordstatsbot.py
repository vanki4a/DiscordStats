...
class Client:
    ...
    def __init__(self, token: str, base_url='https://unbelievable.pizza/api',
                 version=1):
        """\
    ...
    async def get_balance(self, guild: Union[Guild, int], user:
                          Union[User, int]) -> User:
        """\
        GET User Balance as described by API Documentation. Queries user's
        balance.
        Returns a User with the updated balance."""
        if isinstance(user, (int, str)): user = User(user)
        if isinstance(guild, int): guild = Guild(guild)	
        return await self.__request(self.get_balance, [guild, user], 'GET',
                                    f'{guild.id}/users/{user.id}')

    async def get_leaderboard(self, guild: Union[Guild, int]) -> List[User]:
        """\
    ...
    async def edit_balance(self, guild: Union[Guild, int], user:
                           Union[User, int], cash: int = 0, bank: int = 0,
                           reason: str = 'API PATCH') -> User:
        """\
        ...
        if isinstance(user, (int, str)): user = User(user)
        if isinstance(guild, int): guild = Guild(guild)	
        return await self.__request(self.patch_balance, [guild, user, cash, bank],
    ...
    async def set_balance(self, guild: Union[Guild, int], user:
                          Union[User, int], cash: int = 0, bank: int = 0,
                          reason: str = 'API PATCH') -> User:
        """\
